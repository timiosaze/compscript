...
def saveData(file, proxy):
    ...
    for id in ids:
        ...
        while True:
            try:
                response = requests.get('https://www.comparis.ch/immobilien/marktplatz/details/show/' + new_id + '',proxies={'http':proxy, 'https':proxy})
                print(response)
                break
            except requests.exceptions.ProxyError:
                print("Proxy Error Encountered: Reloading")
