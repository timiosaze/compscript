    ...
    with open('/home/compscript/urls.txt', 'r') as reader:
        for line in reader.readlines():
            while True:
                try:
                    url = line.strip()
                    response = requests.get(url, proxies={'http':"http://" + proxy, 'https': "https://" + proxy}, headers={'User-Agent': ua.chrome})
                    break
                except requests.exceptions.ProxyError:
                    print("Proxy Error Encountered: Reloading")
...
def saveData(file, proxy):
    ...
    for id in ids:
        ...
        while True:
            try:
                response = requests.get('https://www.comparis.ch/immobilien/marktplatz/details/show/' + new_id + '',proxies={'http':"http://" + proxy, 'https': "https://" + proxy}, headers={'User-Agent': ua.chrome})
                break
            except requests.exceptions.ProxyError:
                print("Proxy Error Encountered: Reloading")
